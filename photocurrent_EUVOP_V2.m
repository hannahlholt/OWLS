...
% --------- INVERSIONS ----------
plot_N_convergence = 1;         % To plot the converged LOS column number densities with LYRA actual data
plot_inverted_dens = 1;         % To plot the LYRA computed number density and compare with MSIS output
...
N_LOS_i = zeros(num_z_LYRA,1);  % converged LOS colummn number dens for every z_h [1/m^2]
...
for z = (num_z_LYRA:-1:1)
    ...
    N_LOS_i(z) = 2*fzero(f, N_guess, options);
    
    N_guess = N_LOS_i(z);               % use the next guess as the old one
end

% plot the inverted N_LOS data and compare with LYRA 
if plot_N_convergence
    LOS_conv = figure;
    semilogx(N_LOS_i, z_t_LYRA, 'r', n_des_col, z_t_LYRA, 'black')
...
n_meas = (L' * L)^(-1) * L' * N_LOS_i(1:end-1)/2;         % combined number denisty of O and N2 [1/m^3]
n_MSIS = rho_O(:,1) + rho_N2(:,1);                      % combined number density of O and N2 from MSIS [1/m^3]

if true % plot_inverted_dens
    inversion_n = figure;
...
N_LOS_i = zeros(num_z_occ,2);    % converged LOS colummn number dens for every z_h and filter [1/m^2]
N_guess = 1E19;                  % initial guess for LOS column number density (highest altitude)
...
for filt = 1:1      % 1 =  Zr (N2 + O)
    for z = (num_z_occ:-1:z_cutoff)
        ...
        N_LOS_i(z, filt) = 2*fzero(f, N_guess, options);

        N_guess = N_LOS_i(z,filt);                       % use the previous guess as the next one
    end
end
...
if plot_N_convergence
    figure;
    semilogx(N_LOS_i(:,1), MSIS_z_t, 'r', MSIS_N_LOS_comb, MSIS_z_t, 'black')
    ylim([MSIS_z_t(z_cutoff) MSIS_z_t(end)])
    xlabel('LOS Column Number Density [m^{-2}]');
    ylabel('Tangent Ray Point Altitude [km]');
    legend('N_{converged}', 'N_{MSIS}');
    title('Convered MSIS LOS Column Density VS direct MSIS Measurements for O + N2')
    grid on;
end
...
n_meas = (L' * L)^(-1) * L' * N_LOS_i((1:end-1), 1)/2;    % combined number denisty of O and N2 [1/m^3]
n_MSIS = rho_O(:,1) + rho_N2(:,1);                      % combined number density of O and N2 from MSIS [1/m^3]

if plot_inverted_dens
    inversion_n = figure;
    semilogx(n_meas, MSIS_z_t(1:end-1))
    hold on;
    semilogx(n_MSIS, z_atm/1000);
    legend('NRLMSIS-00 Inverted', 'NRLMSIS-00 Direct')
